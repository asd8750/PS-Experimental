...
#>
    ...
    switch ($OldInstName.Substring(0,3)) {
        "PBG" {
            $PlantCode = "PGT" + $OldInstName.Substring(10,1)
            if ($PlantCode -ieq "PGT7") {$PlantCode = "PBG3"}
        }
        "KLM" {
            $PlantCode = "KMT" + $OldInstName.Substring(10,1)
        }
        "DMT" {
    ...
    try {
        ...
        Write-Error "Cannot connect to '$($SvrOld_Name)'"
        Write-Error $_
        Return
    }
    ...
    $oInfo = Invoke-Sqlcmd -ServerInstance $SvrOld_Name -Query "
        WITH MC AS (
            ...
            SELECT	MC.hasMasterKey,
                ...
                FROM MC OUTER APPLY MP; " ;
    $OldInstHasMasterKey = $oInfo.hasMasterKey;   
    if ($OldInstHasMasterKey -eq 1) {
        ...
        Write-Error "--- Old:  Master Cert does NOT exist -- Error"
        Return
    }
    ...
    $nInfo = Invoke-Sqlcmd -ServerInstance $SvrNew_Name -Query "
        WITH MC AS (
            ...
            SELECT	MC.hasMasterKey,
                    ...
                    ISNULL(MP.CertName, 'NONE') AS MPortCertName
                FROM MC OUTER APPLY MP; " ;
    $NewSrv_hasMasterKey = $nInfo.hasMasterKey;   
    ...
    if ($Phase -eq 1 -or $Phase -eq 99) {
        ...
        $oCerts = Invoke-Sqlcmd -ServerInstance $SvrOld_Name -Query "
            SELECT  CT.[name] AS CertName,
                ...
                WHERE ([name] LIKE 'TERA%') AND ([name] NOT LIKE '%$($NewInstName)%')
                ORDER BY CertName; " 
    ...
    if ($Phase -eq 4 -Or $Phase -eq 99) {
        ...
        foreach ($ag in $ClsAGs) {
            ...
            $SqlAG = "
                ...
                BEGIN
                    BEGIN TRY
                        CREATE AVAILABILITY GROUP [$($AGRepl.AGName)]
                    ...
                    END TRY
                    BEGIN CATCH
                        PRINT '*** Error *** Could not CREATE the AG [$($AGRepl.AGName)]'
